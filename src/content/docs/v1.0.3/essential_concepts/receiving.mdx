...
slug: v1.0.3/essential_concepts/receiving
---
...
Wait for a specific type of message (e.g., text, image, document) from the original sender. If you need to know more about the available message types, you can review the examples in [Sending](/v1.0.3/essential_concepts/sending).
...
Waits for the next multimedia message of a specified type (e.g., image, video, document, audio, sticker). The received media will be given as a `Buffer`, similarly to how you would dynamically send images or videos (see [Sending Images](/v1.0.3/essential_concepts/sending#images) and [Sending Videos](/v1.0.3/essential_concepts/sending#videos)).
...
Waits for the next location (ubication) message from the original sender. The resulting object contains the coordinates, similar to what you would use in `SendUbication` (see [Location (Ubication)](/v1.0.3/essential_concepts/sending#location-ubication)).
...
Waits for the next contact (vCard) message from the original sender. This is the exact inverse of sending a contact card; the format is similar to the object used in `SendContact` (see [Contacts (vCard)](/v1.0.3/essential_concepts/sending#contacts-vcard)).
