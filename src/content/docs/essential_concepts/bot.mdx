...
```
The adapter is optional, if you do not provide any, it will use BaileysAdapter by default. You would only use it if
you need to custom, the credentialsFodler, loggermode etc...
...
The `credentialsFolder` and `loggerMode` options have been moved to the WhatsApp Adapter (such as `BaileysAdapter`).
To customize them, pass a configured adapter as the second argument to the `Whatsbotcord` constructor:
```ts
import Whatsbotcord, { BaileysAdapter } from "whatsbotcord";
