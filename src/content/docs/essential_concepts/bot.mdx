...
- **`bot.Commands`**: The built-in command registry. Use `bot.Commands.Add(myCommand)` to register new commands. The arguments you receive there are fully detailed in our [Command Arguments](/essential_concepts/command_args) page.
- **`bot.Use(...)`**: The middleware pipeline manager. You can seamlessly intercept messages before they reach commands by learning about our [Middleware pipeline](/guides/middleware).
- **`bot.Events`**: The global event emitter. Subscribe to socket events or command lifecycle hooks in the [Events system](/guides/events).
- **`bot.SendMsg` & `bot.ReceiveMsg`**: Direct access to the internal sending and receiving managers for advanced cross-chat broadcasting available inside the [Additional API global tools](/essential_concepts/additional_api).
